...
class data_loader:
    ...
    def update_data(self, event):
        ...
        (
            self.placecell_rates_dict,
            self.raw_rates_dict,
            self.placecell_info_dict,
            self.spikes_dict,
            self.behaviour_dict
        ) = self.read_data_file(self.data_set_to_plot)

        # For animal list, fallback to behaviour dict keys if placecell_rates_dict is empty
        if self.placecell_rates_dict:
            self.animals = list(self.placecell_rates_dict.keys())
        elif self.behaviour_dict:
            self.animals = list(self.behaviour_dict.keys())
        else:
            self.animals = []
    ...
    def read_data_file(self, filename):
        file_dir = self.data_exp_dir + filename
        mat_data = loadmat(file_dir, simplify_cells=True)
        rates_dict = mat_data.get("rates_structure", {})

        # Helper to safely create ratemap dicts, returns empty if key missing
        def create_ratemap_dict(rates_subdict):
            if not rates_subdict:
                return {}
            animals = list(rates_subdict.keys())
            sessions = list(rates_subdict[animals[0]].keys())
            placecell_rates = {}
            for mouse in animals:
                session_rates = []
                for sesh in sessions:
                    rates = pd.DataFrame(rates_subdict[mouse][sesh]["rates"])
                    ids = rates_subdict[mouse][sesh]["rates_IDS"]
                    rates.index = ids
                    session_rates.append(rates)
                placecell_rates[mouse] = session_rates
            return placecell_rates

        # Helper to safely create info dicts, returns empty if key missing
        def create_info_dict(info_subdict):
            if not info_subdict:
                return {}
            animals = list(info_subdict.keys())
            sessions = list(info_subdict[animals[0]].keys())
            placecell_info = {}
            for mouse in animals:
                session_info = []
                for sesh in sessions:
                    info = pd.DataFrame(info_subdict[mouse][sesh]["spatial_info"])
                    ids = info_subdict[mouse][sesh]["cell_IDS"]
                    info.index = ids
                    session_info.append(info)
                placecell_info[mouse] = session_info
            return placecell_info

        # Helper to safely create spikes dicts, returns empty if key missing
        def create_spike_dict(spikes_subdict):
            if not spikes_subdict:
                return {}
            animals = list(spikes_subdict.keys())
            sessions = list(spikes_subdict[animals[0]].keys())
            spike_data = {}
            for mouse in animals:
                session_info = []
                for sesh in sessions:
                    spikes = pd.DataFrame(spikes_subdict[mouse][sesh]["spike_times"])
                    spikes.columns = spikes_subdict[mouse][sesh]["cell_ids"]
                    session_info.append(spikes)
                spike_data[mouse] = session_info
            return spike_data

        # Helper to safely create behaviour dicts, returns empty if key missing
        def create_behaviour_dict(behave_subdict):
            if not behave_subdict:
                return {}
            animals = list(behave_subdict.keys())
            sessions = list(behave_subdict[animals[0]].keys())
            behave_data = {}
            for mouse in animals:
                session_info = []
                for sesh in sessions:
                    behaviour = behave_subdict[mouse][sesh]
                    session_info.append(behaviour)
                behave_data[mouse] = session_info
            return behave_data

        placecell_rates_dict = create_ratemap_dict(rates_dict.get("placecell_rates", {}))
        raw_rates_dict = create_ratemap_dict(rates_dict.get("raw_rates", {}))
        placecell_info_dict = create_info_dict(rates_dict.get("placecell_info", {}))
        spikes_dict = create_spike_dict(rates_dict.get("spikes", {}))
        behaviour_dict = create_behaviour_dict(rates_dict.get("behaviour", {}))
...
class animal_widgets:
    ...
    def update_animal_options(self, data):
        self.data = data
        self.selection.options = data.animals_options
        self.ids_to_pool.allowed_tags = data.animals
        self.ids_to_pool.value = data.animals
        self.create_widget(self.output_widget)
    ...
    def on_value_change(self, change):
        if self.selection.value == "pooled data":
            ...
            with self.output_widget:
                clear_output()
                display(self.selection)
                new_image_path = f"{self.data.data_dir}FOV/{self.selection.value}.png"
                display(f"{self.selection.value} field of view")
                new_image_widget = widgets.Image(value=open(new_image_path, "rb").read())
                display(new_image_widget)
